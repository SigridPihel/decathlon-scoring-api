...
13. **Friendly event labels in the results table**: the dropdown already shows friendly labels (e.g. "100 m" instead of `RUN_100M`), since it's fed by `GET /api/decathlon-events`, whose `DecathlonEventDto` includes a `displayName`. The results table doesn't — its `Result` type only carries the raw `event` string, so it currently displays raw enum names. The fix would be a small raw-to-label lookup table on the frontend (e.g. mapping `"RUN_100M"` → `"100 m"`), rather than adding a redundant `displayName` field to the results response too.
14. **Duplication in `PointsCalculator`**: `calculateTrackPoints` and `calculateFieldPoints` both independently call `(int) Math.floor(...)` to truncate the result. Worth extracting into one shared helper function instead of repeating the same rounding logic in two places.
